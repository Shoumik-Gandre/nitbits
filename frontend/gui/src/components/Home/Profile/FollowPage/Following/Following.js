    ...
    const handleFollowing = async() => {
        try {		
            const response = await axios({
                method: 'POST',
                url: `http://127.0.0.1:8000/profiles/userfollows/${match.params.userName}/`,
                headers: {
            ...
            console.log("handleFollowing: ", err);
        }
    }

    const handleUnfollow = async(e) => {
		// unfollow api
        console.log("unfollowing ", e.target.id);
        console.log("from ", match.params.userName);
        let uploadData = new FormData();
        uploadData.append('user', e.target.id);
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                ...
                setFollowing(response.data);
                console.log("following arr : ", response.data);
            } catch (err) {
        ...
        <div>
                            ...
                            <Card className="follower-card-wrap">
                                <Grid container spacing={8}>
                                    ...
                                    <Grid className="follower-action" item xs={2} sm={3}>
                                        <button id={f} onClick={handleUnfollow} className="follower-action-btn">Unfollow</button>
                                    </Grid>
                                </Grid>
                            </Card>
                            <Divider />
            ...
            {/* <Card className="follower-card-wrap">
				<Grid container spacing={8}>
					...
					<Grid className="follower-action" item xs={2} sm={3}>
						<button id="abc" onClick={handleUnfollow} className="follower-action-btn">Unfollow</button>
					</Grid>
				</Grid>
			</Card>
            <Divider />
            <Card className="follower-card-wrap">
				<Grid container spacing={8}>
					...
					<Grid className="follower-action" item xs={2} sm={3}>
						<button id="cde" onClick={handleUnfollow} className="follower-action-btn">Unfollow</button>
					</Grid>
				</Grid>
			</Card> */}
        </div>
